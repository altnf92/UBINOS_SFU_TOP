...
ble_gap_evt_adv_report_t scan_target_result;
...
static void processing_BLE_Central_Scan_Start(BLEPEvt_msgt BLE_evt_msg)
{
	if(BLE_evt_msg.msg != NULL)
	{
		...
		memset(&scan_target_result, 0, sizeof(ble_gap_evt_adv_report_t));
	}
	else
	{
...
bool is_Target_adv_packet(ble_gap_evt_adv_report_t* pPkt)
{

	if(pPkt->data.len < LAP_ADV_DATA_LEN)
		return false;

	if( memcmp(&ble_target_paar_id.scan_target_paar_id[0],&pPkt->data.p_data[LAP_ADV_IDX_PAAR_DEVICE_ID0], 4) == 0)
	{
		return true;
	}else{
		return false;
...
ble_gap_evt_adv_report_t* PAAR_get_ble_scan_target_result()
{
...
void process_ADV_Report(ble_gap_evt_adv_report_t* pPkt)
{
	...
	if(ble_target_paar_id.scan_target_paar_id_enable == true)
	{
		...
		memcpy(&scan_target_result, pPkt, sizeof(ble_gap_evt_adv_report_t));
