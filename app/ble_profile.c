...
#define BLE_UUID_PAAR_SERVICE 				0x2650                      /**< The UUID of the Nordic UART Service. */

#define BLE_UUID_PAAR_TX_CHARACTERISTIC 	0x7F01                      /**< The UUID of the TX Characteristic. */
#define BLE_UUID_PAAR_RX_CHARACTERISTIC 	0x7F02                      /**< The UUID of the RX Characteristic. */

#define BLE_PAAR_MAX_RX_CHAR_LEN        BLE_PAAR_MAX_DATA_LEN        /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_PAAR_MAX_TX_CHAR_LEN        BLE_PAAR_MAX_DATA_LEN        /**< Maximum length of the TX Characteristic (in bytes). */

#define PAAR_BASE_UUID                  {{0x9E, 0xCA, 0xDC, 0x24, 0x0E, 0xE5, 0xA9, 0xE0, 0x93, 0xF3, 0xA3, 0xB5, 0x00, 0x00, 0x40, 0x6E}} /**< Used vendor specific UUID. */
...
//    p_ble_paar->is_notification_enabled = true;
    ...
    err_code = sd_ble_uuid_vs_add(&base_uuid, &p_ble_paar->uuid_type);
    ...
    err_code = tx_char_add(p_ble_paar, p_paar_init);
    VERIFY_SUCCESS(err_code);

    // Add the TX Characteristic.
    err_code = rx_char_add(p_ble_paar, p_paar_init);
    VERIFY_SUCCESS(err_code);
...
uint32_t ble_paar_ble_send_msg(ble_paar_t * p_paar_s, uint8_t * msg, uint16_t length)
{
    ...
    VERIFY_PARAM_NOT_NULL(p_paar_s);

    if ((p_paar_s->conn_handle == BLE_CONN_HANDLE_INVALID))
    {
        return NRF_ERROR_INVALID_STATE;
    }

    if (length > BLE_PAAR_MAX_DATA_LEN)
    {
