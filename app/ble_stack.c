...
static void on_ble_evt(ble_evt_t * p_ble_evt, void * p_context)
{
	...
	switch (p_ble_evt->header.evt_id) {
		...
		case BLE_GATTC_EVT_HVX:
		{
			...
			ble_rx_msgt temp_ble_rx_buffer;

			memset(&temp_ble_rx_buffer, 0, sizeof(ble_rx_msgt));

			temp_ble_rx_buffer.evt = BLE_CENTRAL_EVT;
			temp_ble_rx_buffer.conn_handle = conn_handle;

			if(pPkt->len >= PAAR_MAXIMUM_PACKET_SIZE)
			{
				temp_ble_rx_buffer.len = PAAR_MAXIMUM_PACKET_SIZE;
				memcpy(&temp_ble_rx_buffer.msg, &pPkt->data, PAAR_MAXIMUM_PACKET_SIZE);
			}
			else
			{
				temp_ble_rx_buffer.len = pPkt->len;
				memcpy(&temp_ble_rx_buffer.msg, pPkt->data, pPkt->len);
			}

			msgq_send(ble_rx_msgq, (uint8_t*)&temp_ble_rx_buffer);
			break;
		}
		...
		case BLE_GATTS_EVT_WRITE:
		{
			...
			if( p_wpkt->handle == m_ble_paar.tx_handles.cccd_handle && (p_wpkt->len==2) ){
				...
				temp_ble_rx_buffer.evt = BLE_PERIPHERAL_EVT;
				temp_ble_rx_buffer.conn_handle = conn_handle;

				if(p_wpkt->len >= PAAR_MAXIMUM_PACKET_SIZE)
				{
					temp_ble_rx_buffer.len = PAAR_MAXIMUM_PACKET_SIZE;
					memcpy(&temp_ble_rx_buffer.msg, p_wpkt->data, PAAR_MAXIMUM_PACKET_SIZE);
				}
				else
				{
					temp_ble_rx_buffer.len = p_wpkt->len;
					memcpy(&temp_ble_rx_buffer.msg, p_wpkt->data, p_wpkt->len);
				}
